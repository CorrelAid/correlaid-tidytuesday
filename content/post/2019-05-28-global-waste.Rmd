...
- [GitHub](https://github.com/pmeiners)
...
```{r, warning=FALSE}
p <- ggplot(data = data_time, aes(x = Year, y = reorder(cntry, -waste_total_total), z = gdp_pcap_change)) + 
  geom_tile(aes(fill = total_pop_change)) +
  scale_fill_viridis(limits = c(-3, 10),oob = scales::squish)
ggplotly(p, height = 2000)
...
```{r, warning=FALSE}
p <- ggplot(data = data_time, aes(x = Year, y = reorder(cntry, -waste_mis_pcap), z = gdp_pcap_change)) + 
